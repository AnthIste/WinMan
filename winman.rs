...
use win32::win32_macros::{LOWORD,HIWORD};
...
fn main() {
    ...
    match dummy_window {
        Ok(window) => {
            ...
            // Signal exit
            let msg = CString::from_slice(b"All done!");
            let title = CString::from_slice(b"Exiting");

            MessageBoxA(0 as HWND, msg.as_ptr(), title.as_ptr(), 0);
        }
        Err(code) => {
            let msg = CString::from_slice(format!("We couldn't create a window becase of {} :<", code).as_bytes());
            let title = CString::from_slice(b"Exiting");
